    ...
    <div className='bg-white rounded-lg shadow p-4 space-y-4'>
     <div>
      ...
      <p className="flex items-center gap-2 text-sm text-gray-600 mt-1">
        ...
        {data.paymentMethod=="online"?<p className="gap-2 text-sm text-gray-600">Payment:
        {data.payment?"true" : "false"}</p> : <p className="gap-2 text-sm text-gray-600">Payment Method:
     ...
     <p className="text-sm text-gray-600">Lat: {data?.deliveryAddress?.latitude},
       Lon: {data?.deliveryAddress?.longitude}</p>
     </div>

      {/* user item ketna order kiya oh sab */}
        <div className='flex space-x-4 overflow-x-auto pb-2'>
            {data?.shopOrders?.shopOrderItems?.map((item, _index)=>(
