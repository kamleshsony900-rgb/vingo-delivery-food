 ...
 useEffect(()=>{
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      <button className='absolute top-4 left-4 z-10 flex items-center gap-2 bg-black/50 hover:bg-black/70 text-white px-3 py-2 rounded-full shadow transition cursor-pointer'
      onClick={()=>navigate("/")}>
      <FaArrowLeft/>
        <span>Back</span>
      </button>
      ...
      {shop && <div className='relative w-full h-64 md:h-80 lg:h-96'>
         ...
         <div className='absolute inset-0 bg-linear-to-b from-black/30 to-black-30 flex flex-col justify-center items-center text-center px-4'>
         <FaStore className='text-white text-4xl mb-3 drop-shadow-md'/>
         <h1 className='text-3xl md:text-5xl font-semibold text-white drop-shadow-lg'>{shop?.name}</h1>

         <div className='flex items-center gap-2'>
           <FaLocationDot size={16} className='text-red-500'/>
         <p className='text-lg text-gray-200 font-medium'>{shop?.address}</p>
         </div>
         </div>
        </div>}
